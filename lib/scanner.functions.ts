...
export const runScan = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ScanResult> => {
    ...
    if (!apiKey) throw new Error("NOVAIN API key is not configured.");

    const response = await fetch(`${baseUrl}/api/public/v1/verify/website`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
      body: JSON.stringify({ target: data.url }),
    });
    const payload = await response.json();
    if (!response.ok) throw new Error(payload?.error?.message || "NOVAIN verification failed.");

    const score = payload.assessment?.score ?? 0;
    ...
    return {
      ...
      durationMs: null,
      overallScore: score,
      scores: { headers: score, cookies: score, tls: score, sessions: score, csrf: score, xss: score, recon: score },
      headers: [], cookies: [], tls: null, csrf: [], xss: [], sessions: [], recon: [],
      evidence: { primary: null, setCookies: [], forms: [], mixedContentRefs: [], xssProbe: null, corsProbe: null, exposure: [], meta: [], redirect: null, crtsh: null },
      error: null,
